 ...
 */
function patternLayout(layoutConfig, globalConfig, worker) {
	...
	var formatters = {
		...
		mm: function(level, now) {
			return padZero(now.getMonth(), 2);
		},
		ss: function(level, now) {
			return padZero(now.getSeconds(), 2);
		},
		level: function(level, now, msg) {
			return levels[level];
		},
		levelc: function(level, now, msg) {
		...
		line: function(level, now, msg, args) {
		...
		linec: function(level, now, msg, args) {
			var stack = new Error().stack;
			if (stack) {
				stack = stack.split('\n')[5];
				if (stack && stack.match(/\/([^\/]+?[0-9:]+)/)) {
